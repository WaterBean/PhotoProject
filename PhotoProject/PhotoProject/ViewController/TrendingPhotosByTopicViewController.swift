...
final class TrendingPhotosByTopicViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        NetworkManager.shared.fetchTopicPhotos(topic: .goldenHour) {
            self.photoList1 = $0
        }
        NetworkManager.shared.fetchTopicPhotos(topic: .businessWork) {
            self.photoList2 = $0
        }
        NetworkManager.shared.fetchTopicPhotos(topic: .architectureInterior) {
            self.photoList3 = $0
        }
...
extension TrendingPhotosByTopicViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        vc.photo = item
        NetworkManager.shared.fetchStatistics(id: item.id) {
            vc.response = $0
